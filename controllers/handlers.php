...
trait validImage
{
	 ...
	 * Its size should be no more 00000 byte.
	 */
	...
	protected static function isImage( $image )
	{
		...
		{
			$errorMessages = [
				UPLOAD_ERR_INI_SIZE   => 'File size exceeds 0.18 MB! (code 1)',
				UPLOAD_ERR_FORM_SIZE  => 'File size exceeds 0.18 MB! (code 2)',
		...
		if ( filesize( $filePath ) > $limitBytes ) return array( 'File size exceeds 0.18 MB!' );
...
final class ChangeAvatarHandler extends Base
{
	...
	protected function showSuccess( $template )
	{
		...
		$avatarAbsPath = $_SERVER['DOCUMENT_ROOT'] . '/cloud/avatars/' . $name . $format;

		$avatarAbsHTMLPath = '/cloud/avatars/' . $name . $format;
